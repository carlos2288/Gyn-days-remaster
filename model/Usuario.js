export default class Usuario{
    constructor(nome = 'carlos',nascimento = '26/11/1999',matricula='20181015030158',endereco = 'Rua Jandira, 1100', 
    contato = '+55985319265',imagem = 'data:image/jpeg;base64,',
    objetivo = 'ficar bombado', frequencia = 3, orientador = 'jorge', dataInicio = '26/11/1999',
    peso = 20.5, altura = 1.80, partEsq = 0, partDir = 0,coxaDir = 0,
    coxaEsq = 0, quadril = 0, abdomem = 0, cintura = 0, torax = 0, braçoEsq = 0, braçoDir = 0, 
    ABracoDir = 0, ABracoEsq = 0, punhoEsq = 0, PunhoDir = 0,listaExercicios = []){
        
        this.valido = false
        this.nome = nome
        this.email = ''
        this.senha = ''
        this.nascimento = nascimento
        this.matricula = matricula
        this.endereco = endereco
        this.contato = contato
        this.objetivo = objetivo
        this.frequencia = frequencia
        this.orientador = orientador
        this.dataInicio = dataInicio
        this.peso = peso
        this.altura = altura
        this.partEsq = partEsq
        this.partDir = partDir
        this.coxaEsq = coxaEsq
        this.coxaDir = coxaDir
        this.quadril = quadril
        this.abdomem = abdomem
        this.cintura = cintura
        this.torax = torax
        this.braçoEsq = braçoEsq
        this.braçoDir = braçoDir
        this.ABracoDir = ABracoDir
        this.ABracoEsq = ABracoEsq
        this.punhoEsq = punhoEsq
        this.PunhoDir = PunhoDir
        this.listaExercicios = listaExercicios
        this.imagem = imagem
    }
}