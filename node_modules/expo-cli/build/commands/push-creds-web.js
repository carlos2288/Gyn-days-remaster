...
function _default(program) {
  program.command('push:web:upload [path]').description('Upload VAPID key pair and VAPID subject for web push notifications.').helpGroup('notifications').option('--vapid-pubkey [vapid-public-key]', 'URL-safe base64-encoded VAPID public key.').option('--vapid-pvtkey [vapid-private-key]', 'URL-safe base64-encoded VAPID private key.').option('--vapid-subject [vapid-subject]', vapidSubjectDescription).asyncActionProjectDir(async (projectDir, options) => {
  ...
  program.command('push:web:generate [path]').description('Generate VAPID key pair for web push notifications.').helpGroup('notifications').option('--vapid-subject [vapid-subject]', vapidSubjectDescription).asyncActionProjectDir(async (projectDir, options) => {
  ...
  program.command('push:web:show [path]').description('Log the VAPID public key, the VAPID private key, and the VAPID subject currently in use for web notifications for this project.').helpGroup('notifications').asyncActionProjectDir(async projectDir => {
    const {
      args: {
        remotePackageName
      }
  ...
  program.command('push:web:clear [path]').description('Delete previously uploaded VAPID public key, VAPID private key, and VAPID subject.').helpGroup('notifications').asyncActionProjectDir(async projectDir => {
...
async function _uploadWebPushCredientials(projectDir, options) {
  var _appJson$expo, _appJson$expo$notific;
  ...
  if (((_appJson$expo = appJson.expo) === null || _appJson$expo === void 0 ? void 0 : (_appJson$expo$notific = _appJson$expo.notification) === null || _appJson$expo$notific === void 0 ? void 0 : _appJson$expo$notific.vapidPublicKey) && appJson.expo.notification.vapidPublicKey !== results.vapidPublicKey) {
...
//# sourceMappingURL=push-creds-web.js.map