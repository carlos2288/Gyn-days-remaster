...
const printUsage = async (projectDir, options = {}) => {
  ...
  const iosInfo = process.platform === 'darwin' ? `${b`i`} to run on ${u`i`}OS simulator (${b`shift+i`} to select the simulator model)` : '';
  const webInfo = `${b`w`} to run on ${u`w`}eb`;
  const platformInstructions = [androidInfo, iosInfo, webInfo].filter(Boolean).map(instructions => ` \u203A Press ${instructions}.`).join('\n');
...
const startAsync = async (projectRoot, options) => {
  ...
  _xdl().Simulator.setInteractiveCallback(async pause => {
    if (pause) {
  ...
  _xdl().Android.setInteractiveCallback(async pause => {
    if (pause) {
      stopWaitingForCommand();
    } else {
      startWaitingForCommand();
        ...
        case 'i':
          {
            ...
            (0, _log().default)('Opening in iOS simulator...');
            await _xdl().Simulator.openProjectAsync({
              projectRoot,
              shouldPrompt: false
            });
